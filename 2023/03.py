schematic = """...766.......821.547.....577......................................387.....................56..........446.793..........292..................
...
# For each line find the number indexes
lines = schematic.split("\n")

# Pre-process by padding first and last line with non-special chars
lines.insert(0, '.' * 140)
lines.append('.' * 140)
...
# Iterate through lines skipping the padded first and last line
for line_idx in range(1, len(lines)):
    print(line_idx)
    line = lines[line_idx]
    
    # Obtain tuples of numbers in string and their starting index
    numbers = [(match.start(), match.group()) for match in re.finditer("\d+", line)]

    for (char_idx, num) in numbers:
        # print(f"number {num} at idx {char_idx}")
        left_idx = 0 if char_idx == 0 else char_idx - 1
        right_idx = 139 if (char_idx + len(num) > 139) else char_idx + len(num)
        idx_range = range(left_idx, right_idx+1)
        # print(f"left_idx {left_idx}, right idx {right_idx}")
        upper_line_chars = [lines[line_idx-1][range_idx] for range_idx in idx_range]
        bottom_line_chars = [lines[line_idx+1][range_idx] for range_idx in idx_range]
        
        is_part = is_special_char(line[left_idx]) or is_special_char(line[right_idx]) \
                    or True in [is_special_char(char) for char in upper_line_chars] or True in [is_special_char(char) for char in bottom_line_chars]
        
        if (is_part):
            part_numbers.append(int(num))
        
print(f"Sum of part numbers: {np.sum(part_numbers)}")