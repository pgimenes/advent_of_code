...
for line in input.split("\n"):
    [game_id, trials] = line.split(":")
    game_id = int(game_id[5:])
    trials = trials.split(";")

    game_possible = True

    # Minimum value of cubes for each colour that must've been present for this game to be valid
    min_vals = {
        "red": 0,
        "green": 0,
        "blue": 0,
    ...
    for trial in trials:
        # Get boxes per colour in this trial
        colours = trial.split(",")

        for colour in colours:
            # Get how many boxes were obtained for this colour in this trial
            num = int(re.findall("\d+", colour)[0])

            # Check if any of the rules is violated
            for key, val in rules.items():
                if key in colour:
                    if num > min_vals[key]:
                        min_vals[key] = num

                    # This trial had a higher number of cubes for this colour than allowed
                    if num > val:
                        game_possible = False

    total_power += np.prod([val for _, val in min_vals.items()])

    # After analysing all the trials, append to list of valid game IDs
    if game_possible:
        valid_games.append(game_id)

print(f"Sum of valid game IDs: {np.sum(valid_games)}")
